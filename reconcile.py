...
def parse_option_details(gallo_name: str) -> dict:
    ...
    if not gallo_name:
        return {}
    match = re.match(
        r"([A-Z0-9]+)\s*\((C|V)\)\s*([\d,\.]+)", gallo_name.replace(".", "")
    )
    if not match:
        return {}
    return {
...
class ExchangeRateLoader:
    ...
    def get_rate(self, date, asset_type: str):
        rate_df = (
            self.dolar_mep
            if asset_type in ["ACCION", "RF", "OPCION"]
            else self.dolar_ccl
        )
        if rate_df.empty:
            return None
...
def reconcile_portfolio():
    ...
    try:
        ...
        if "broker_transaction_id" in open_pos_df.columns:
            processed_ids.update(
                open_pos_df["broker_transaction_id"].dropna().astype(str)
            )
    except (FileNotFoundError, pd.errors.EmptyDataError):
        logging.info(
            f"{OPEN_POSITIONS_FILE} not found or is empty. Assuming no existing open positions."
        )
    ...
    try:
        ...
        if "buy_broker_transaction_id" in closed_trades_df.columns:
            processed_ids.update(
                closed_trades_df["buy_broker_transaction_id"].dropna().astype(str)
            )
        if "sell_broker_transaction_id" in closed_trades_df.columns:
            processed_ids.update(
                closed_trades_df["sell_broker_transaction_id"].dropna().astype(str)
            )
    except (FileNotFoundError, pd.errors.EmptyDataError):
        logging.info(
            f"{CLOSED_TRADES_FILE} not found or is empty. Assuming no existing closed trades."
        )
    except Exception as e:
        logging.error(
            f"An unexpected error occurred while reading {CLOSED_TRADES_FILE}: {e}"
        )
    ...
    for tx in all_transactions:
        tx_id = str(tx.get("id"))
        if tx.get("state") != "FULFILLED" or tx.get("orderOperation") not in [
            "BUY",
            "SELL",
        ]:
            continue
        if tx_id not in processed_ids:
            try:
                ...
                ticker, currency = (
                    tx.get("symbol") or instrument.get("name"),
                    tx.get("currency"),
                )
                ...
                clean_tx = {
                    ...
                    "currency": currency,
                    "fees": abs(
                        float(tx.get("total", 0)) - float(tx.get("totalGross", 0))
                    ),
                }
                ...
                processed_ids.add(
                    tx_id
                )  # Añadir al set para manejar duplicados dentro del mismo JSON
            except Exception as e:
                logging.warning(
                    f"Could not process new transaction {tx.get('id')}: {e}"
                )
    ...
    logging.info(
        f"Processing {len(new_transactions)} new FULFILLED BUY/SELL transactions..."
    )
    ...
    try:
        ...
        for col in date_cols:
            if col in open_positions_df.columns:
                ...
                open_positions_df[col] = pd.to_datetime(
                    open_positions_df[col], errors="coerce"
                )

        open_positions_df.rename(
            columns={
                "purchase_date": "date",
    ...
    for tx in new_transactions:
        ...
        if not rate:
            logging.warning(
                f"No exchange rate for {tx['ticker']} on {tx['date'].date()}"
            )
        ...
        if tx["op_type"] == "BUY":
            ...
            for lot in matching_lots:
                ...
                proportion = (
                    qty_from_lot / lot["quantity"] if lot["quantity"] > 0 else 0
                )
                ...
                newly_closed_trades.append(
                    {
                        ...
                        "total_revenue_ars": (revenue_ars or 0)
                        * (qty_from_lot / tx["quantity"])
                        if tx["quantity"] > 0
                        else 0,
                        "total_revenue_usd": (revenue_usd or 0)
                        * (qty_from_lot / tx["quantity"])
                        if tx["quantity"] > 0
                        else 0,
    ...
    open_df = pd.DataFrame(open_positions).rename(
        columns={
            "date": "purchase_date",
    ...
    if "purchase_date" in open_df.columns:
        open_df["purchase_date"] = pd.to_datetime(
            open_df["purchase_date"], errors="coerce"
        )
    if "expiration_date" in open_df.columns:
        open_df["expiration_date"] = pd.to_datetime(
            open_df["expiration_date"], errors="coerce"
        )

    open_df[final_open_cols].to_csv(
        OPEN_POSITIONS_FILE, index=False, date_format="%Y-%m-%d"
    )
    logging.info(
        f"Reconciliation complete. {len(open_df)} open lots written to {OPEN_POSITIONS_FILE}."
    )
    ...
    if newly_closed_trades:
        ...
        file_exists = (
            os.path.exists(CLOSED_TRADES_FILE)
            and os.path.getsize(CLOSED_TRADES_FILE) > 0
        )
        ...
        new_closed_df[final_closed_cols].to_csv(
            CLOSED_TRADES_FILE,
            mode="a",
        ...
        logging.info(
            f"{len(new_closed_df)} new trades appended to {CLOSED_TRADES_FILE}."
        )
